...
st.set_page_config(
    page_title="Tracker de Carrera | UAI", page_icon="🎓", layout="wide"
)
...
def save_to_github(updated_df):
  if "GITHUB_TOKEN" not in st.secrets or "REPO_NAME" not in st.secrets:
    # Si no están configurados los secrets, guarda solo de forma local
    updated_df.to_csv("materias.csv", index=False)
    return
  ...
  if res.status_code == 200:
    ...
    data = {
        "message": "Actualización automática de notas desde Streamlit",
...
col2.metric(
    "Materias Aprobadas", f"{aprobadas} / {total}", f"{porcentaje*100:.1f}%"
)
...
search_query = st.text_input(
    ...
    placeholder="Ej: Algoritmos, Física, Bases de Datos...",
).strip()


# 6. Función para renderizar materias
    ...
    with st.expander("✏️ Editar estado / nota"):
      ...
      with c_btn:
        ...
        if st.button("Guardar", key=f"btn_{m_id}", use_container_width=True):
          ...
          df.loc[df["id"] == m_id, "nota"] = (
              nueva_nota if nuevo_estado == "Aprobada" else 0
          )
          save_to_github(df)
          st.cache_data.clear()
          st.rerun()
